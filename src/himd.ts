...
import { HiMDFile, HiMDFilesystem } from './filesystem/himd-filesystem';
import { getBytesPerFrame, HiMDCodec } from './codecs';
import iconv from 'iconv-lite';
import { getFramesPerBlock } from './trackinfo';
import { HiMDBlockStream, HiMDMP3Stream, HiMDNonMP3Stream } from './streams';
import { createMasterKey, getMP3EncryptionKey, initCrypto } from './encryption';
...
function encode(encoding: string, content: string){
	try{
		return new Uint8Array(getErroringEncoder(encoding).write(content));
	}catch(ex){
		return null;
	}
}
...
export class HiMD {
	...
	protected async openDatanumDependent(name: string, mode?: "rw" | "ro"){
		return this.filesystem.open(`/hmdhifi/${name}${this.datanum!.toString(16).padStart(2, '0')}.hma`, mode);
	}
	...
	getGroup(groupIndex: number): HiMDRawGroup {
		...
		// TODO: Make sure we're reading correct data and not garbage.
		return {
	...
	@dirty writeTrack(trackSlotIndex: number, track: HiMDRawTrack){
		assert(trackSlotIndex >= 0 && trackSlotIndex <= 2047);
	...
	@dirty addString(string: string, type: HiMDStringType): number{
		...
		const encodingOrder: [string, HiMDStringEncoding][] = [
			["latin1", HiMDStringEncoding.LATIN1],
			["sjis", HiMDStringEncoding.SHIFT_JIS],
			["utf16-be", HiMDStringEncoding.UTF16BE],
		];
		
		for(let [name, enc] of encodingOrder) {
			let uEncodedText;
			if((uEncodedText = encode(name, string)) !== null){
				encodedText = Array.from(uEncodedText);
				// Append the information about the encoding
				encodedText.splice(0, 0, enc);
				break;
			}
		}
		if(encodedText === null)
	...
	async openNonMP3Stream(trackNumber: number){
		...
		const masterKey = createMasterKey(track.ekbNumber, track.key);
