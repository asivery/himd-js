...
export class SonyVendorUSMCDriver extends USBMassStorageDriver {
    async preventAllowMediumRemoval(status: 'prevent' | 'allow'){
        const command = new Uint8Array([
            0x1e, 0x0, 0x0, 0x0, status === 'prevent' ? 0xFF : 0x0,
        ]);
    ...
    async getTime(){
        return await this.sendCommandInGetResult(new Uint8Array([
            0xC2, 0x00, 0x00, 0x50, 0x00, 0x00, 0x00, 0x00, 0x07
        ]), 0x7, false, 0xc);
    }

    async setTime(/*time*/){
        const command = new Uint8Array([
            0xc2, 0x00, 0x00, 0x90, 0x00, 0x02, 0x17, 0x13, 0x02, 0x06, 0x6d, 0x00, 0x00, 0x00, 0x00, 0x00
        ]);
    ...
    private async drmRead(param: number, length: number){
        const command = new Uint8Array([
            0xa4, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xbd, (length >> 8) & 0xff, length & 0xff, param, 0x00, 0x00, 0x00, 0x00, 0x00
        ]);
    ...
    private async drmWrite(param: number, data: Uint8Array){
        ...
        const command = new Uint8Array([
            0xa3, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xbd, (length >> 8) & 0xff, length & 0xff, param, 0x00, 0x00, 0x00, 0x00, 0x00
        ]);
    ...
    async getAuthenticationStage2Info(){
        ...
        assert(arrayEq(key, [0x6a, 0x7a, 0x4c, 0x7d, 0x5f, 0x3f, 0x86, 0x84, 0x28, 0x6d, 0x1a, 0x12, 0x32, 0x98, 0x22, 0x13]), "Key is not the default");
    ...
    async writeAuthenticationStage3Info(hostMac: Uint8Array){
        ...
        finalBuffer.set([
            ...
            0x00, 0x01, 0x00, 0x12,
            0x00, 0x00, 0x00, 0x00,
            0xec, 0xd2, 0x63, 0xdb, 0xfe, 0xc8, 0x3d, 0xd3, 0x25, 0x28, 0x4b, 0x7b, 0x8c, 0xe4, 0xdf, 0xd1
        ], 10);
...
export class UMSCHiMDSession {
    ...
    public async performAuthentication(){
        ...
        const recalculatedMac = retailMac(concatUint8Arrays([
            discId, this.hostNonce, deviceNonce
        ]), MAIN_KEY);
        assert(arrayEq(mac, recalculatedMac), "Device MAC doesn't match!");

        // Create the host MAC
        const hostMac = retailMac(concatUint8Arrays([
            discId,
            deviceNonce,
            this.hostNonce
        ]), MAIN_KEY);
        ...
        const sessionKey = retailMac(concatUint8Arrays([
            discId,
            mac,
            hostMac
        ]), MAIN_KEY);
...
export class UMSCHiMDFilesystem extends HiMDFilesystem {
    ...
    async _list(path: string): Promise<HiMDFilesystemEntry[]> {
        const dirContents = await new Promise<string[]>((res, rej) => 
            this.fatfs.readdir(
                ...
                (err: any, files: string[]) => err ? rej(err) : res(files)
            )
        );
        ...
        for(let f of dirContents){
            let name: string = join(path, f),
                type: 'directory' | 'file';
            try{
                const stats = await new Promise<any>((res, rej) => this.fatfs.stat(join(this.rootPath, path, f), (err: any, stats: any) => err ? rej(err) : res(stats)));
    ...
    async open(filePath: string, mode: "rw" | "ro" = 'ro'): Promise<HiMDFile> {
        ...
        const fd = await new Promise<number>((res, rej) => this.fatfs.open(path, mode == 'ro' ? 'r' : 'r+', (err: any, fd: number) => err ? rej(err) : res(fd)));
        return new UMSCHiMDFile(
            this,
            mode === 'rw',
            fd,
            stat.size
    ...
    async rename(path: string, newPath: string){
        ...
        if(exists){
            const newName = Array(8).fill(0).map(() => String.fromCharCode(Math.floor((Math.random() * (90 - 65)) + 65))).join('') + ".WMD";
    ...
    async getSize(path: string) {
        const stats = await new Promise<any>((res, rej) => this.fatfs.stat(join(this.rootPath, path), (err: any, stats: any) => err ? rej(err) : res(stats)));
        return stats.size;
    }
}
