...
export function dumpTrack(himd: HiMD, trackSlotNumber: number, externalDecryptor?: CryptoProvider): { data: DumpingGenerator, format: "MP3" | "WAV" | "OMA" } {
...
export async function uploadMP3Track(
    ...
    let idxTitle = 0,
        ...
        idxArtist = 0;
    if(title)
        idxTitle = himd.addString(title, HiMDStringType.TITLE);
    if(album)
        idxAlbum = himd.addString(album, HiMDStringType.ALBUM);
    if(artist)
        idxArtist = himd.addString(artist, HiMDStringType.ARTIST);
