...
export function getBytesPerFrame(ci: CodecInfo) {
    ...
    switch (codecId) {
        ...
        case HiMDCodec.ATRAC3PLUS_OR_MPEG:
            if (isMpeg(codecInfo)) {
                let mask = ~0;
                if ((codecInfo[3] & 0xc0) === 0xc0) mask = ~3;
...
export function getSamplesPerFrame({ codecId, codecInfo }: CodecInfo) {
    switch (codecId) {
        ...
        case HiMDCodec.ATRAC3PLUS_OR_MPEG:
            if (!isMpeg(codecInfo)) return SONY_ATRAC3P_SAMPLES_PER_FRAME;
            if ((codecInfo[3] & 0x30) == 0x30) return MPEG_I_SAMPLES_PER_FRAME;
            else return MPEG_II_III_SAMPLES_PER_FRAME;
        default:
...
export function getSampleRate({ codecId, codecInfo }: CodecInfo) {
    ...
    if (codecId === HiMDCodec.ATRAC3PLUS_OR_MPEG && !isMpeg(codecInfo)) codecId = HiMDCodec.ATRAC3;
    switch (codecId) {
        case HiMDCodec.LPCM:
            return 44100;
...
export function getCodecName(ci: CodecInfo): HiMDCodecName {
    switch (ci.codecId) {
        ...
        case HiMDCodec.ATRAC3:
            return 'AT3';
        case HiMDCodec.ATRAC3PLUS_OR_MPEG:
            if (isMpeg(ci.codecInfo)) return 'MP3';
            else return 'A3+';
        default:
